...
const DateInput: React.FC<DateInputProps> = ({ value, onChange, className }) => {
  ...
  const handleKeyDown = (field: keyof DateParts) => (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowRight') {
      ...
      if (
        ...
        e.preventDefault();
        if (field === 'day') monthRef.current?.focus({ preventScroll: true });
  ...
  return (
    <div className={cn('flex items-center rounded-xl border-primary-500 border-b px-2 text-sm', className)}>
      <input
        type="text"
        ref={monthRef}
        max={12}
        ...
        className="w-6 border-none bg-white p-0 text-center text-primary-500 outline-none"
        placeholder="M"
      />
      ...
      <input
        ...
        className="w-7 border-none bg-white p-0 text-center text-primary-500 outline-none"
        placeholder="D"
      />
      <span className="-mx-px opacity-20">/</span>
      <input
        ...
        className="w-12 border-none bg-white p-0 text-center text-primary-500 outline-none"
        placeholder="YYYY"
      />
    </div>
  );
